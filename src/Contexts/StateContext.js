    ...
    const Mint_ = async(counter) =>{
      if(typeof window.ethereum !=='undefined')
      {
           if(currentAccount)
           {
            ...
            let Ammount = counter;
            let value=(Ammount*1000000000).toString()+"000000"
            console.log(value)
            let overrides = {
              value: value,
              from:  currentAccount,
            ...
            try{
                const Transaction = await Base.mint(Ammount, overrides);
                setLoading(true);
  ...
  //************||PreSaleMint||*************/
   ...
   const GetInfo = async() =>{
      ...
      const Base = new ethers.Contract(contractAdress, contractABI, signer);
        ...
        let Price_        = await Base.cost();
        let PricePresale_ = await Base.cost();
        let SaleStep_     = await Base.maxMintAmount();
        let MaxSupply_    = await Base.maxSupply();
