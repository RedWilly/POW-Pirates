...
contract POWFarm is Ownable, Pausable {
    ...
    function fund(uint256 _amount) external onlyOwner onlyAfterInit {
        ...
        rewardToken.transferFrom(address(msg.sender), address(this), _amount.sub(leftOver));
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken) external onlyOwner onlyAfterInit {
        massUpdatePools();
    ...
    function deposit(uint256 _pid, uint256 _amount) external onlyAfterInit whenNotPaused {
        ...
        if (userAmount > 0) {
            uint256 pendingAmount = userAmount.mul(accERC20PerShare).div(1e36).sub(userInfo[_pid][msg.sender].rewardDebt);
        ...
        userInfo[_pid][msg.sender].rewardDebt = userAmount.mul(accERC20PerShare).div(1e36);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external whenNotPaused {
        ...
        require(userAmount >= _amount, "withdraw: can't withdraw more than deposit");
        ...
        uint256 pendingAmount = userAmount.mul(accERC20PerShare).div(1e36).sub(userRewardDebt);
