...
contract('Farm with Deflation tokens', ([owner, alice, bob, carl]) => {
    before(async () => {
        this.bep20 = await BEP20.new("Mock token", "MOCK", 1000000);
        ...
        this.farm = await Farm.new(this.bep20.address, 100, this.startBlock);
        this.farm.add(15, this.lp.address);

        await this.bep20.approve(this.farm.address, 10000);
    ...
    describe('when created', () => {
        it('is linked to the Mock BEP20 token', async () => {
            const linked = await this.farm.rewardToken();
            assert.equal(linked, this.bep20.address);
        });
    ...
    describe('with a participant withdrawing after 70 blocks', () => {
        ...
        it('gives alice 3749 MOCK and 1500 LP', async () => {
            const balanceBEP20 = await this.bep20.balanceOf(alice);
            assert.equal(3749, balanceBEP20);
    ...
    describe('with a participant partially withdrawing after 80 blocks', () => {
        ...
        it('gives carl 2800 MOCK and 1485 LP', async () => {
            const balanceBEP20 = await this.bep20.balanceOf(carl);
            assert.equal(2800, balanceBEP20);
    ...
    describe('when it receives more funds (8000 MOCK)', () => {
        before(async () => {
            await this.bep20.approve(this.farm.address, 8000);
    ...
    describe('with an added lp token (for 25%) after 100 blocks', () => {
        before(async () => {
            ...
            this.farm.add(5, this.lp2.address);
        });

        it('has a total reward of 3451 MOCK pending', async () => {
            const totalPending = await this.farm.totalPending();
            assert.equal(3351, Number(totalPending));
        });
    ...
    describe('with a participant partially withdrawing LP2 after 150 blocks', () => {
        ...
        it('gives carl 498 MOCK and 200 LP', async () => {
            const balanceBEP20 = await this.bep20.balanceOf(carl);
            assert.equal(3299, balanceBEP20);
    ...
    describe('with participants withdrawing after closed', async () => {
        ...
        it('gives alice 1250 MOCK and 981 LP2', async () => {
            const balanceBEP20 = await this.bep20.balanceOf(alice);
            assert.equal(4999, balanceBEP20);
        ...
        it('gives carl 5511 MOCK and 491 LP', async () => {
            const balanceBEP20 = await this.bep20.balanceOf(bob);
            assert.equal(5511, balanceBEP20);
        ...
        it('gives carl 4000 MOCK and 500 LP', async () => {
            const balanceBEP20 = await this.bep20.balanceOf(carl);
            assert.equal(7237, balanceBEP20);
        ...
        it('has an end balance of 250 MOCK, which is lost forever', async () => {
            ...
            const balanceFarm = await this.bep20.balanceOf(this.farm.address);
