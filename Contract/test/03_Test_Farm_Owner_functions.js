...
contract('Farm Special Functions', ([owner, alice, bob, carl, adminWallet, adminWallet2]) => {
    before(async () => {
        ...
        this.farm = await Farm.new(this.bep20.address, toWei('100'), this.startBlock);
        // withdrawal fee of 5% (argument/1000)
        this.farm.add(15, this.lp.address);

        await this.bep20.approve(this.farm.address, toWei('5000050'));
        await this.farm.fund(toWei('5000050'));
    });
    ...
    describe('Deployments and before initializations', async () => {
        it('Deploys without init', async () => {
            this.farm2 = await Farm.new(zeroAddress, 0, 0);
        });

        it('Fails to fund', async () => {
            await this.bep20.approve(this.farm2.address, toWei('5000000'));
        ...
        it('Fails to add', async () => {
            await truffleAssert.reverts(
                this.farm2.add(15, this.lp.address),
                "Farm: init the farm first"
            );
        });
        ...
        it('Initialized normally ', async () => {
            ...
            await this.farm2.initializeFarm(toWei('10'), this.startBlock2);
        ...
        it("Only initializes once in it's life time", async () => {
            await truffleAssert.reverts(
                this.farm2.initializeFarm(toWei('10'), this.startBlock2),
    ...
    describe('before the start block', () => {
        ...
        it('Reducing rewards per block push the end block to the future and avoids division precision loss', async () => {
            const balanceBefore = await this.bep20.balanceOf(owner);
            await this.farm.changeRewardPerBlock(toWei('60'));

            const adminBalance = await this.bep20.balanceOf(owner);
            //assert.equal(adminBalance - balanceBefore, toWei('20'));
    ...
    describe('Widrawal after 70 blocks', async () => {
        ...
        it('should claim 3912.75 Rewards for Alice', async () => {
            const aliceBalance = await this.bep20.balanceOf(alice);
            assert.equal(toWei("3912.75"), aliceBalance);
        });

        it('should send 249.75 Rewards to admin as claimFee', async () => {
            const adminBalance = await this.bep20.balanceOf(adminWallet);
            assert.equal(toWei("449.75"), adminBalance); // 200 Rewards was already in wallet
        });
    ...
    describe('Change widrawal address', async () => {//adminWallet2
        before(async () => {
            await this.farm.changeFeeWallet(adminWallet2, {from: owner});
        });

        it('Should withdraw to the new admin wallet', async () => {
            await this.farm.withdraw(0, toWei('500'), {from: bob});

            const adminWallet2Balancec = await this.bep20.balanceOf(adminWallet2);
